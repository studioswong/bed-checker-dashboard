...
const AddManagerForm = props => {
  ...
  const handleHospital = e => {
    const hospitalId = props.hospitals.filter(
      hospital => hospital.name == e.target.innerText
    )[0].id;
    setHospitalId(hospitalId);
    setSelectedHospital(e.target.innerText);
  };
  const [addHospitalManager, { data, error, loading }] = useMutation(
    CREATE_HOSPITAL_MANAGER_MUTATION
  );
  const submitForm = e => {
    e.preventDefault();
                    ...
                    >
                      ...
                      <DropdownMenu>
                        {props.hospitals.map(hospital => {
                          return (
                            <div>
                              <DropdownItem onClick={e => handleHospital(e)}>
                                {hospital.name}
                              </DropdownItem>
